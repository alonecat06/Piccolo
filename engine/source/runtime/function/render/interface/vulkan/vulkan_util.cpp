...
namespace Piccolo
{
    ...
    static EShLanguage FindLanguage(const RHIShaderStageFlagBits shader_type) {
        switch (shader_type) {
        case RHI_SHADER_STAGE_VERTEX_BIT:
            return EShLangVertex;
        case RHI_SHADER_STAGE_TESSELLATION_CONTROL_BIT:
            return EShLangTessControl;
        case RHI_SHADER_STAGE_TESSELLATION_EVALUATION_BIT:
            return EShLangTessEvaluation;
        case RHI_SHADER_STAGE_GEOMETRY_BIT:
            return EShLangGeometry;
        case RHI_SHADER_STAGE_FRAGMENT_BIT:
            return EShLangFragment;
        case RHI_SHADER_STAGE_COMPUTE_BIT:
    ...
    bool VulkanUtil::compileShader(const RHIShaderStageFlagBits shader_type, const char* pshader, std::vector<unsigned char>& spirv_char)
    {
        ...
        for (auto val : spirv)
        {
            spirv_char.push_back(static_cast<unsigned char>(val >> 4));
            spirv_char.push_back(static_cast<unsigned char>(val));
        }
